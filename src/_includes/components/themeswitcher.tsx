...
export default function (props) {
  const [themeValue, updateThemeValue] = typeof window !== 'undefined' ? useState(localStorage.theme) : useState("unity-auto");
  return (
    <dialog
      id="themeSwitcher"
    ...
    >
      ...
      />
      <LDAThemeTile
        bgClassLight="bg-stone-100 text-black"
        cardClassLight="bg-stone-200"
        bgClassDark="bg-stone-900 text-white"
        cardClassDark="bg-stone-800"
        primaryClass="bg-yellow-400 text-white hover:bg-yellow-200"
        themeBasename="nimbus"
...
function ThemeTile({
  ...
  theme = "none",
  name = "Theme",
  cardClass = "bg-gray-500",
}) {
  return (
    <div
      class={
        "flex-grow lg:col-span-1 " +
        bgClass +
        " rounded-md " +
        (active ? "border-primary-3" : "border-scheme-3") +
        " border-2 m-2 min-w-48 w-full h-36 relative hover:shadow-lg active:shadow-2xl transition-shadow"
      }
      onClick={() => {
    ...
    >
      <div
        class={cardClass + " rounded-md absolute top-4 left-4 p-2 select-none"}
      >
        {name}
      </div>
      <button
        class={
          primaryClass + " rounded-md absolute right-4 bottom-4 p-4 select-none"
        }
      >
        Primary
      </button>
...
function LDAThemeTile({
  active = false,
  updateThemeValue = (value: string) => {},
  mode = "light",
  bgClassLight = "bg-white",
  bgClassDark = "bg-black",
  primaryClass = "bg-red-500",
  themeBasename = "none",
  name = "Theme",
  cardClassLight = "bg-gray-500",
  cardClassDark = "bg-gray-500",
}) {
  ...
  return (
    ...
    >
      ...
      <div class="rounded-md absolute right-4 bottom-4 select-none">
        <button
          class={primaryClass + " rounded-md rounded-r-none p-4 has-tooltip"}
          aria-label="Automatic theme, from system preference"
          onClick={(e) => {
            localStorage.theme = themeBasename + "-auto";
            X_Apply_Theme();
            updateThemeValue(themeBasename + "-auto");
          }}
        >
          <div
            ...
            aria-hidden
          >
            Automatic theme, from system preference
          </div>
          <Icon icon="ph:circle-half-fill" color="currentColor" />
        </button>
        <button
          class={primaryClass + " p-4 has-tooltip"}
          aria-label="Light theme"
          onClick={(e) => {
            localStorage.theme = themeBasename + "-light";
            X_Apply_Theme();
            updateThemeValue(themeBasename + "-light");
          }}
        >
          <div
            class="tooltip rounded-md shadow-lg p-1 bg-scheme-3 mt-8 ml-2 -translate-x-[50%] text-onscheme-3"
        ...
        <button
          class={primaryClass + " rounded-md rounded-l-none p-4 has-tooltip"}
          aria-label="Dark theme"
          onClick={(e) => {
            localStorage.theme = themeBasename + "-dark";
            X_Apply_Theme();
            updateThemeValue(themeBasename + "-dark");
          }}
        >
          <div
            class="tooltip rounded-md shadow-lg p-1 bg-scheme-3 mt-8 ml-2 -translate-x-[50%] text-onscheme-3"
